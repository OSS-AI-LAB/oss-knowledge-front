<template>
  ...
  >
    ...
    <!-- 하단 아이콘들 -->
    <div class="mt-auto flex flex-col items-center pb-4 space-y-2 border-t border-gray-200 pt-2">
      <!-- 사용자 아바타 -->
      <button
        @click="openUserMenu"
  ...
  >
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto">
        <!-- 빈 상태 -->
        <div v-if="conversationStore.filteredConversations.length === 0" class="p-6 text-center">
          ...
          <p class="text-sm text-gray-500">
            {{ conversationStore.searchQuery ? '검색 결과가 없습니다' : '아직 대화가 없습니다' }}
          </p>
        </div>
            ...
            <div class="pr-8">
              <h3
                :class="[
                  ...
                  conversationStore.currentConversationId === conv.id ? 'text-gray-900' : 'text-gray-700'
                ]"
              >
                {{ conv.title }}
              </h3>
        ...
        <div class="p-4">
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium text-gray-900 truncate">NJ</p>
              <p class="text-xs text-gray-500 truncate">nj@example.com</p>
            </div>
...
const expandForSearch = () => {
  ...
  // 다음 틱에서 검색 입력창에 포커스
  setTimeout(() => {
    const searchInput = document.querySelector('input[placeholder="대화 검색..."]')
    if (searchInput) {
      searchInput.focus()
    }
  }, 300) // transition 시간 후
